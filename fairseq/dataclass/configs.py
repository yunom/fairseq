...
class CommonConfig(FairseqDataclass):
    ...
    tensorboard_logdir: Optional[str] = field(
        default=None,
        metadata={"help": "path to save logs for tensorboard, should match --logdir of running tensorboard (default: no tensorboard logging)"},
    )
    ...
    on_cpu_convert_precision: bool = field(
        default=False,
        metadata={"help": "if set, the floating point conversion to fp16/bf16 runs on CPU. This reduces bus transfer time and GPU memory usage."},
    )
    ...
    suppress_crashes: bool = field(
        default=False,
        metadata={
            "help": "suppress crashes when training with the hydra_train entry point so that the main method can return a value (useful for sweeps)"
        },
    )
...
class DistributedTrainingConfig(FairseqDataclass):
    ...
    ddp_backend: DDP_BACKEND_CHOICES = field(default="pytorch_ddp", metadata={"help": "DistributedDataParallel backend"})
    ...
    slowmo_momentum: Optional[float] = field(
        default=None,
        metadata={"help": "SlowMo momentum term; by default use 0.0 for 16 GPUs, 0.2 for 32 GPUs; 0.5 for 64 GPUs, 0.6 for > 64 GPUs"},
    )
...
class OptimizationConfig(FairseqDataclass):
    ...
    sentence_avg: bool = field(
        default=False,
        metadata={"help": "normalize gradients by the number of sentences in a batch (default is to normalize by number of tokens)"},
    )
    ...
    skip_remainder_batch: Optional[bool] = field(
        default=False,
        metadata={"help": "if set, include the last (partial) batch of each epoch in training (default is to skip it)."},
    )
...
class CheckpointConfig(FairseqDataclass):
    ...
    continue_once: Optional[str] = field(
        default=None,
        metadata={"help": "continues from this checkpoint, unless a checkpoint indicated in 'restore_file' option is present"},
    )
    ...
    load_checkpoint_on_all_dp_ranks: bool = field(
        default=False,
        metadata={"help": "load checkpoints on all data parallel devices (default: only load on rank 0 and broadcast to other devices)"},
    )
    write_checkpoints_asynchronously: bool = field(
        default=False,
        metadata={
            "help": ("Write checkpoints asynchronously in a separate thread. NOTE: This feature is currently being tested."),
...
class GenerationConfig(FairseqDataclass):
    ...
    max_len_a_mt: float = field(
        default=0,
        metadata={"help": "generate sequences of maximum length ax + b, where x is the source length for the first-pass decoder"},
    )
    ...
    lenpen_mt: float = field(
        default=1,
        metadata={"help": "length penalty for the first-pass decoder: <1.0 favors shorter, >1.0 favors longer sentences"},
    )
    unkpen: float = field(
        default=0,
    ...
    retain_dropout_modules: Any = field(
        default=None,
        metadata={"help": "if set, only retain dropout for the specified modules; if not set, then dropout will be retained for all modules"},
    )
...
class CommonEvalConfig(FairseqDataclass):
    ...
    post_process: Optional[str] = field(
        default=None,
        metadata={
            "help": ("post-process text by removing BPE, letter segmentation, etc. Valid options can be found in fairseq.data.utils.post_process."),
    ...
    model_overrides: str = field(
        default="{}",
        metadata={"help": "a dictionary used to override model args at generation that were used during model training"},
    )
...
class EvalLMConfig(FairseqDataclass):
    ...
    context_window: int = field(
        default=0,
        metadata={"help": "ensures that every evaluated token has access to a context of at least this size, if possible"},
    )
    softmax_batch: int = field(
        default=sys.maxsize,
        metadata={"help": "if BxT is more than this, will batch the softmax over vocab to this amount of tokens, in order to fit into GPU memory"},
    )
